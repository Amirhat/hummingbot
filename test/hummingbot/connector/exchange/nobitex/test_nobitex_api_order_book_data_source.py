...
# TODO: check this file completely to make sure it is correct with the nobitex api


class NobitexAPIOrderBookDataSourceUnitTests(unittest.TestCase):
    ...
    def _snapshot_response(self):
        resp = {
            "lastUpdateId": 1027024,  # 1737911434054 in nobitex
    ...
    @aioresponses()
    def test_get_new_order_book_successful(self, mock_api):  # TODO: First check done
    ...
    @aioresponses()
    def test_get_new_order_book_raises_exception(self, mock_api):  # TODO: First check done
        url = web_utils.public_rest_url(path_url=CONSTANTS.SNAPSHOT_PATH_URL, domain=self.domain)
        regex_url = re.compile(f"^{url}".replace(".", r"\.").replace("?", r"\?"))
    ...
    @aioresponses()
    @patch(
        "hummingbot.connector.exchange.nobitex.nobitex_api_order_book_data_source"
