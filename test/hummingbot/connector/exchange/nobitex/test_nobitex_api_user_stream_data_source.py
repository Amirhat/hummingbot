...
class NobitexUserStreamDataSourceUnitTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.auth = NobitexAuth(api_key="TEST_API_KEY", time_provider=self.mock_time_provider)
        ...
        self.data_source = NobitexAPIUserStreamDataSource(
            auth=self.auth,
    ...
    def test_get_listen_key_log_exception(self, mock_api):
        url = web_utils.private_rest_url(path_url=CONSTANTS.NOBITEX_USER_STREAM_PATH_URL, domain=self.domain)
    ...
    @patch(
        "hummingbot.connector.exchange.nobitex.nobitex_api_user_stream_data_source.NobitexAPIUserStreamDataSource"
        "._ping_listen_key",
